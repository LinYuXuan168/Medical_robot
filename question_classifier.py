...
class QuestionClassifier:
    ...
    def classify(self, question):
        data = {}  # 定义保存返回结果的字典
        medical_dict = self.check_medical(question)  # ac自动机提取question中的关键词
        if not medical_dict:  # 如果没有提取到关键词，函数结束，返回空字典
            return {}

        data['args'] = medical_dict  # 保存关键词
        # 收集问句当中所涉及到的实体类型
        types = []
        ...
        if check_words(self.food_qwds + self.cure_qwds, question) and 'food' in types:
            ...
            if deny_status:
                question_type = 'food not disease'
            else:
        ...
        if question_types == [] and 'symptom' in types:
            question_types = ['symptom disease']
...
if __name__ == '__main__':
    handler = QuestionClassifier()#定义对象
    question=input('input an question:')#接收用户输入的问题
    data=handler.classify(question)#进行分类并返▣结果
    print(data)#打印结果
