class QuestionPaser:
    ...
    def parser_main(self, res_classify):
        ...
        entity_dict = self.build_entitydict(args)  # 调用上方定义的方法返回问句中实体类型及其对应的实体关键词
        question_types = res_classify['question_types']  # 返回问题类型

        sqls = []  # 用来保存最终的cypher查询语句
        for question_type in question_types:  # 遍历所有的问题类型
            sql_ = {}
            sql_['question_type'] = question_type
            sql = []
            if question_type == 'disease_symptom':  # 判断问题类型
                sql = self.sql_transfer(question_type, entity_dict.get('disease')) # 返回对应的cypher查询语句
...
if __name__ == '__main__':
    ...
    data = {'args': {'感冒': ['disease']}, 'question_types': ['disease_do_food']}
    res = handler.parser_main(data)
    print(res)
